...
tpl = ('apple',) # NOTE: To create tuples with just one value, the comma is necessary!
not_tuple = ('apple')
...
tpl = ('a', 'b', 'c')
el1, el2, el3 = tpl
print(el1, el2, el3) # a b c

# Umpack *
tpl = ('a', 'b', 'c', 'd', 'e')
left, *middle, right = tpl
print(left, middle, right) # a ['b', 'c', 'd'] e NOTE: The umpack generated a list, not a tuple
...
for i in range(len(tpl)):
    print(tpl[i], end=', ') # a, b, c, d, e, 
print()

# For each
tpl = ('a', 'b', 'c', 'd', 'e')
for el in tpl:
    print(el, end=', ') # a, b, c, d, e, 
print()

# For each with index
tpl = ('a', 'b', 'c', 'd', 'e')
for idx, el in enumerate(tpl):
    print(f'{idx}: {el}', end=', ') # 0: a, 1: b, 2: c, 3: d, 4: e, 
print()
...
# Cast to list
# Tuples are not changeable, so, to change data the tuple must be cast to list first
tpl = (1, 2, 3)
lst = list(tpl)
lst[0:] = ['a', 'b', 'c']
tpl = tuple(lst)
